...
namespace Model.Neg
{
    ...
    {
        //objClienteDao consegue acessar tudo da ClienteDao
        private ClienteDao objClienteDao;

        public ClienteNeg()
        {
            //Instancia para usar a ClienteDao
...
//O nome precisa ser <= 30 e > 0. O Trim retira os espaços

                verificacao = nome.Length <= 30 && nome.Length > 0;
                // ! é negação, 
 // !verificacao significa que se a verificar não for verdadeira, vai retornar um erro.
                if (!verificacao)
                {
                    objCliente.Estado = 2;
                    return;
